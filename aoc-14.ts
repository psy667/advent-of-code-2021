import { max, min, read, sum, AoC } from "./utils"
    
type ParsedData = { seq: string, map: [string, string][] }
export class AoC14 implements AoC<ParsedData> {
    day = 14;
    prod = true;

    public async run() {
        const parsedData = this.parse(await read(this.day, this.prod))
        console.time('part 1')
        console.log(this.part1(parsedData));
        console.timeEnd('part 1')
        console.time('part 2')
        console.log(this.part2(parsedData));
        console.timeEnd('part 2')
    }

    private parse(raw: string): ParsedData {
        const lines = raw
            .split('\n');
    ...
    public part1(data: ParsedData) {
        {
            ...
            for (let i = 0; i < 10; i++) {
                result = this.splitByTwo(result).map((it, idx) => {
                    if (idx) {
    ...
    public part2(data: ParsedData) {
        ...
        this.splitByTwo(seq).map(it => result[it] = (result[it] || 0) + 1);
